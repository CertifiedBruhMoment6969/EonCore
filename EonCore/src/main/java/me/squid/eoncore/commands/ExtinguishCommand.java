        ...
        if (args.length == 0){
            if (sender instanceof Player){
                ...
                if (p.hasPermission(getPermissionNode())){
                    p.setFireTicks(0);
                    p.sendMessage(Utils.chat(plugin.getConfig().getString("Extinguish-Message")));
                } else {
            ...
            if (target != null && sender.hasPermission(getOthersPermNode())){
                ...
                target.sendMessage(Utils.chat(plugin.getConfig().getString("Target-Extinguish-Message")));
                sender.sendMessage(Utils.chat(plugin.getConfig().getString("Extinguish-Other-Message")));
            }
        }
